...
#SBATCH --output=./results/logs/%x-%j.out           # output file name
...
export base_vlm=./data/pretrained_models/Qwen2.5-VL-3B-Instruct # must be a local path, due to simpler will run in other where
export base_vlm=./data/pretrained_models/Qwen3-VL-4B-Instruct-Action
export freeze_module_list=""
export action_input_dim=2560
export DIT_TYPE="DiT-B"
export config_yaml=./configs/train_recipes/QwenGR00T_Robocasa.yaml
export dataset_mix=fourier_gr1_unified_1000
export include_state=True
export output_root_dir=./results/training
export run_id=debug_1224_${dataset_mix}_${Framework_name}_nostate_qwen3
...
export output_dir=${output_root_dir}/${run_id}
mkdir -p ${output_dir}
