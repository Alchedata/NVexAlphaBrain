...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_yaml",
        type=str,
        default="configs/train_recipes/QwenFM_OXE.yaml",
    ...
    # Support _model_config_ reference: load model config first, then override with training config
    if '_model_config_' in cfg:
        model_cfg_path = cfg.pop('_model_config_')
        model_cfg = OmegaConf.load(model_cfg_path)
        cfg = OmegaConf.merge(model_cfg, cfg)

    dotlist = normalize_dotlist_args(extra_cli_args)
